...
import { ShoppingCart, Minus, Plus, Trash2, X, CheckCircle2, PackageCheck, MapPin, Search, Loader2 } from "lucide-react"
...
export function Cart() {
    ...
    const handleCheckout = async () => {
        ...
        try {
            // Helper to get raw price from item
            const getRawPrice = (item: any) => parseFloat(item.price.replace('₱', '').replace(',', ''))
            ...
            // 2. Update user profile & Send Chat Message
            if (supabaseUser?.id) {
            ...
            const chatMsg = `I would like to checkout:\n${cartDetails}\nDelivery Fee: ${formatCurrency(deliveryFee)}\nTotal (incl. Fee): ${total}\n\n${productImages}`

            const { error: chatError } = await supabase.from('messages').insert([
                {
                    content: chatMsg,
                    sender_id: supabaseUser?.id || user?.phone || "anonymous",
    ...
    return (
        <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
            ...
            <SheetContent className="flex w-full flex-col sm:max-w-md bg-background border-l-border">
                <SheetHeader className="space-y-2 pr-6 pb-2 border-b border-border">
                ...
                {itemCount === 0 ? (
                    ...
                    <>
                        ...
                        <div className="space-y-2 pt-4 border-t border-border bg-card/50 -mx-6 px-6 pb-4">
                            <div className="space-y-2">
                                <div className="space-y-2 mb-4">
                                    ...
                                    <div className="relative group/input">
                                        <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-2 pr-3 border-r border-border/50">
                                            <span className="text-xs font-black text-muted-foreground">+63</span>
                                        </div>
                                        <input
                                            type="tel"
                                            value={phoneNumber}
                                            ...
                                            maxLength={11}
                                            className="w-full h-11 bg-muted/10 border border-border rounded-xl pl-16 pr-4 text-xs font-black focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all outline-none group-hover/input:border-primary/20"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 mb-2">
                                    <div className="flex items-center justify-between">
                                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Find Place</label>
                                    </div>
                                    <form onSubmit={handleSearch} className="relative group/search">
                                        <input
                                            ...
                                            className="w-full h-11 bg-muted/10 border border-border rounded-xl pl-10 pr-4 text-xs font-black focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all outline-none"
                                        />
                                ...
                                <div className="rounded-xl overflow-hidden border border-border/80 shadow-sm bg-background ring-offset-background transition-all focus-within:ring-2 focus-within:ring-primary/20 h-[180px]">
                                    <LocationPicker
                                ...
                                {!deliveryLocation && (
                                    <div className="bg-destructive/10 border border-destructive/20 p-1.5 rounded-lg flex items-center justify-center gap-2">
                                        <X className="h-3 w-3 text-destructive" />
                                        <p className="text-[8px] text-destructive font-black uppercase tracking-widest">
                            ...
                            <SheetFooter className="pt-2">
                                <Button
                                    className={cn(
                                        "w-full py-5.5 text-base font-black uppercase tracking-widest shadow-lg shadow-primary/10 transition-all active:scale-[0.98]",
                                        (isBanned || !deliveryLocation || phoneNumber.length < 10 || isProcessing) && "opacity-50 grayscale cursor-not-allowed"
                                    )}
                                    size="lg"
                                    onClick={handleCheckout}
                                    disabled={isBanned || !deliveryLocation || phoneNumber.length < 10 || isProcessing}
                                >
                                    {isProcessing ? <Loader2 className="h-5 w-5 animate-spin" /> : 
                                     isBanned ? `Restricted (${remainingTime}m)` : 
                                     !deliveryLocation ? "Select Location First" : 
                                     "Checkout"}
                                </Button>
                            </SheetFooter>
                        </div>
